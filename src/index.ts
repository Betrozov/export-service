...
app.post('/', (req: Request, res: Response): any => {
  ...
  if (typeof screenshotRequest.eventName === 'undefined') {
    customEventIsLoaded = true;
  }
  ...
  Chrome.New((err: Error, newTabData: any) => {
    ...
    Chrome((chromeInstance: any) => {
      ...
      Page.loadEventFired(() => {
        ...
        getEventStatusByName(Runtime.evaluate, screenshotRequest.eventName, (error: Error, data: {isLoaded: boolean}) => {
          if (error) {
...
function getEventStatusByName(runtimeEvaluate: Function, eventName: string, cb: Function): Function {
  return getPrerenderReadyStatus(runtimeEvaluate, eventName, (error: Error, data: {isLoaded: boolean}): Function | Timer => {
    if (error) {
      return cb(error);
    }
    ...
    return setTimeout((): Function => {
      return getEventStatusByName(runtimeEvaluate, eventName, cb);
    }, 100);
  });
}

function getPrerenderReadyStatus(runtimeEvaluate: Function, eventName: string, cb: Function): Function {
  return runtimeEvaluate({
    'expression': `window.${eventName}`
    ...
    if (response.result.type === 'undefined') {
      return cb(`The custom event '${eventName}' is not found`);
    }
